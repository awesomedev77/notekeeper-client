...
const PinnedTask = ({ pinTaskList, handleEdit, handleCompletebtn, handleDeletebtn }) => {
    ...
    const handleUnPin = id => {
        ...
        const url = `http://localhost:5000/unpin/task/${id}`
        fetch(url, {
            method: "put"
        })
            .then(resp => resp.json())
    ...
    return (
        <div className='bg-yellow-100 rounded-xl p-4'>
            <h1 className='text-center text-3xl font-bold text-purple-700 '>Pinned Task</h1>
            {
                pinTaskList?.map(task => <div key={task._id} className='bg-white grid grid-cols-12 gap-4 content-between p-4 mt-4 rounded-lg shadow hover:shadow-lg hover:bg-indigo-200 outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150 '>

                    <div onClick={() => { handleEdit(task._id, task.title, task.description) }} className='col-span-7 md:col-span-9 lg:col-span-10'>
                    ...
                    <div className='col-span-5 md:col-span-3 lg:col-span-2 justify-items-end'>
                        <div className='flex-wrap lg: inline-flex justify-end gap-1'>
                            ...
                            {/* Edit btn */}
                            <button onClick={() => { handleUnPin(task._id) }} className="w-full bg-purple-300 hover:text-white  hover:bg-purple-400 text-gray-800 font-bold py-2 px-4 rounded inline-flex items-center focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150">
