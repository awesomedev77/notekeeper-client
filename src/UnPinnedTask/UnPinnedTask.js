...
const UnPinnedTask = ({
  ...
  //pin button event
  const handlePin = (id) => {
    const url = `${process.env.REACT_APP_URL}/task/pin/${id}`;
    fetch(url, {
      method: "put",
    })
      .then((resp) => resp.json())
      .then((data) => {
        if (data?.data?.modifiedCount > 0) {
          ...
          swal(
            "Task Pinned!",
            "WoW your task is pinned successfully",
            "success"
          );
        } else {
          swal(
            "Task Pinned Error!",
            "An error occured during the pin operation",
            "error"
          );
        }
      });
